...
Promoted from a consumer document-extraction pipeline where the same
pattern was duplicated across the vision cache, the pipeline manifest,
and the heading normalizer's checkpoint files.

Usage::

    from pf_core.utils.io import atomic_write_text, atomic_write_json

    atomic_write_json(Path("./manifest.json"), {"step": "vision", "n": 42})
