...
LLM agents are unreliable at calendar arithmetic. When a newsletter says
"the president on Tuesday signed the order" the model often guesses wrong
...
Design rules:
  ...
  - Future dates are unresolvable — a newsletter cannot describe events
...
Originally extracted from a research-aggregation app where the
curated-extractor LLM emits these phrases verbatim from newsletter prose.
