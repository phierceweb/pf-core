...
Lists of trusted domains, prohibited sources, controlled tags, controlled action types, scope filters, curated newsletter sources — all data, not framework. Hand them to prompts via builders that read from config:
...
def _build_tier1_domain_block() -> str:
    ...
    domains = MY_PROJECT.evidence_tiers[0].get("trusted_domains", [])
...
2. **Expose it on the project's domain config object** with a sensible project-neutral default. The default should be honest — typically a phrase like `"the body whose actions this project tracks"`, not a placeholder string.
...
## Worked example — a research-aggregation app

A research-aggregation consumer ships with `researchapp.yaml` as its project config layer. The same principles in this doc are applied:

| Project-specific concern | Lives at |
|---|---|
| Display name of the app | `researchapp.yaml:name` (rendered via `cfg.RESEARCHAPP_NAME` / `@@RESEARCHAPP_NAME@@`) |
| Noun phrase naming the body whose actions are tracked | `researchapp.yaml:subject` (rendered via `{subject}` / `@@SUBJECT@@`) |
| Temporal scope start | `researchapp.yaml:start_date` (rendered via `cfg.START_DATE`) |
| Trusted Tier-1 government / court domains | `researchapp.yaml:evidence_tiers[0].trusted_domains` (rendered via `{tier1_domain_block}`) |
| Trusted Tier-2 journalism domains | `researchapp.yaml:evidence_tiers[1].trusted_domains` |
| Prohibited primary-source domains | `researchapp.yaml:prohibited_sources` / `prohibited_domains` |
| Curated newsletter source list | `researchapp.yaml:curated_sources` |
| Controlled-tag vocabulary | `researchapp.yaml:tags` |
| Action-type vocabulary | `researchapp.yaml:action_types` |
| Scope filter actor names | `researchapp.yaml:scope_filters` |
