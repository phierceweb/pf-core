...
| New consumer project, full app framework | Pin `pf-core[full,<dialect>]~=0.13.0` in `pyproject.toml`, then `pip install -e .` |
...
|---|---|
| Full-stack web app, Postgres | `pf-core[full,llm,postgres]~=0.13.0` |
| Full-stack web app, MySQL + article ingest | `pf-core[full,llm,mysql,articles]~=0.13.0` (+ `[redis,ratelimit]` if caching/limits are used) |
| Full-stack web app, SQLite | `pf-core[full,llm]~=0.13.0` (SQLite driver is stdlib) |
| Batch document pipeline (no web/db) | `pf-core[image-phash,tracking,llm]~=0.13.0` |
| Foundation-only CLI (no LLM at all) | `pf-core[cli]~=0.13.0` |
