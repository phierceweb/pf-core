...
canonical_url("https://www.example.com/story?utm_source=substack&utm_medium=email")
...
canonical_url("https://apnews.com/article/foo?utm_source=substack")
...
A "trusted domain" short-circuit (skipping liveness for known-good evidence-tier sites). That list is project policy, not framework infrastructure. Wrap `check_url_cached` with a 5-line consumer-side wrapper when you need it.
...
**Research-aggregation app** — `app/utils/sources.py` re-exports from pf-core:
