...
| [LLM step](llm-step.md) | `llm_step` — the per-item batch hot path as one call: cache lookup (hit short-circuits) → budget gate → `tracked_messages_call` → parse/validate → cache store. The batch shell (Job, `run_parallel`, persistence) stays in the caller. |
...
| [Markdown export](export.md) | `MarkdownExporter` — turn a system-of-record into a markdown tree for RAG/review, incrementally (write-if-changed + atomic + scoped orphan prune, `force_prune_dirs` for stable subdirs, `check()` dry-run freshness gate for committed trees). Includes a `yaml_frontmatter` helper. |
| [Env-var resolver](env.md) | `resolve_int` / `resolve_str` / `resolve_bool` / `resolve_positive_int` — explicit-arg → env var → default with warn-on-malformed, plus a `>= min_value` bounded variant. Codifies the `config-driven.md` rule. |
