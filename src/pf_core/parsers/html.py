"""HTML body extractor for newsletter / article content.
...
Designed for the curated-ingest shape: a post arrives as HTML, we hand
the LLM the plain text for event extraction and the link list with
context so the model can identify which event a candidate primary-source
URL backs.

Used by a consumer's Substack and RSS+HTML parsers; lifted to pf-core
in 0.19.0 after the implementations converged byte-identically.
...
# plain-text buffer. Not exhaustive — good enough for newsletter prose,
...
class BodyExtractor(HTMLParser):
    ...
    - ``link_records``: list of ``(start_offset_in_joined_text, url,
      ...
      filtered (newsletter platforms sprinkle these for layout).
    """
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if tag == "a" and self._link_stack:
            ...
            # empty shells (newsletter platforms sprinkle these for layout).
            if url and anchor:
