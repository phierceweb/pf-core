...
The validity model is mtime-based plus optional run-record flag matching:
  ...
  - Its mtime must be >= the latest file matching any upstream-glob spec
    (e.g., `<output_dir>/.vision-cache/*.json` — catches "an intermediate
...
Promoted from a consumer document-extraction pipeline
(`orchestrators/_resume.py`). The helpers return `str | None` (raw
snapshot content); consumers hydrate their own result types from that
content.
"""
