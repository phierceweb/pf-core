...
heading list, the current set of section IDs, the current paragraph
order, …) and produces a transform plan. By the time you APPLY the
plan, the underlying data may have drifted — other transforms ran
between gather and apply, the input was edited, etc. Blindly applying
the plan against the new state can mis-target — rename the wrong
heading, mutate the wrong paragraph, drop the wrong section.
...
`_heading_normalize.NormalizeData` + `apply_normalization` pair,
generalized so any LLM-driven transform pipeline can use the same
safety pattern.
...
Usage::
    ...
    def gather_renames(headings: list[Heading]) -> GatherResult[dict[int, str]]:
        plan = llm_propose_renames([h.text for h in headings])
        return GatherResult(
            target_count=len(headings),
            target_texts=tuple(h.text for h in headings),
            data=plan,
        )

    # Phase 2 — apply (possibly much later, after other transforms have run)
    def apply_renames(md: str, gathered: GatherResult[dict[int, str]]) -> str:
        current = extract_headings(md)  # re-extract NOW, not at gather time
        result = safe_apply(
            gathered,
            current_texts=[h.text for h in current],
            apply_fn=lambda plan: rewrite_headings(md, current, plan),
            label="rename_headings",
        )
        return result if result is not None else md  # drift fallback
"""
    ...
    *,
    ...
    Args:
        ...
        label: Identifying label for log events. Lets multiple
            ...
            (e.g., ``"heading_normalize"``, ``"rename_sections"``).
