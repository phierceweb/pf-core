...
PostgreSQL. They previously used delete-then-insert, which took gap locks on the
secondary index and deadlocked under concurrent writers — replaced 2026-06-24.
"""
...
def _is_mysql_deadlock(exc: BaseException) -> bool:
    ...
    The ``_record_once`` upsert no longer takes the secondary-index gap locks
    the old delete-then-insert path deadlocked on, but concurrent writers of
    the same ``(llm_run_id, validator)`` key (or FK locks on ``llm_runs``) can
    still occasionally make InnoDB kill one transaction with ``(1213,
    'Deadlock found when trying to get lock; try restarting transaction')``.
    The losing transaction is safe to retry verbatim — the upsert is idempotent.
    """
