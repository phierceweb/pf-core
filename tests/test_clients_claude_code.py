...
class TestChatHappyPath:
    ...
    def test_extra_args_inserted_before_print(self, mock_which, mock_run):
        ...
        cmd = mock_run.call_args.args[0]
        assert cmd[1:4] == ["--allowedTools", "Bash", "--print"]
...
class TestModelOverride:
    ...
    devastating for batch consumers (a research-aggregation app ran into
    ...
    def test_model_lands_after_extra_args(self, mock_which, mock_run, monkeypatch):
        """Order: [binary, *extra_args, --model X, --print, prompt]. Putting
        --model in extra_args manually still works (caller's choice), but
        the per-instance/per-call model uses this canonical position so
        ...
        client.chat(messages=[{"role": "user", "content": "x"}])
        cmd = mock_run.call_args.args[0]
        # Expected: [binary, --allowedTools, Bash, --model, haiku, --print, prompt]
        assert cmd[1:6] == ["--allowedTools", "Bash", "--model", "haiku", "--print"]
