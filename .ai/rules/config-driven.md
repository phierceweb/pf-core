...
A real incident: a hardcoded `_CLAUDE_CODE_TIMEOUT_S = 600` in a consumer's heading-normalize service timed a large conversion out at exactly 10 minutes, shipping a broken document and forcing a source edit + re-run. An env var would have been a one-line `.env` change — and wanting to bump the constant for one run *was* the signal it should have been operational all along.
